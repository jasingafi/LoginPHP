...
//Extraer los datos de las asignaturas de cada alumno
$materias = "SELECT u.idusuarios,m.idmateria, m.nombre_materia, m.unidades
              FROM usuarios AS u INNER JOIN materias AS m
              ON u.idusuarios = m.idusuario
      WHERE u.idusuarios = '$iduser'";
$resultadomaterias = $conexion->query($materias);

  $cant = "SELECT COUNT(m.idmateria) as 'Cantidad' FROM alumnos AS a
            INNER JOIN usuarios AS u ON a.IdAlumnos = u.id_alumno
            INNER JOIN materias AS m ON u.idusuarios = m.idusuario
            WHERE u.idusuarios= '$iduser' ";
  $resultadocant = $conexion->query($cant);

//Editar alumnos
$ID = $_GET["id"];
$sqlu = "SELECT u.*, a.*, t.* FROM alumnos AS a
            INNER JOIN usuarios AS u ON a.IdAlumnos = u.id_alumno
            INNER JOIN tipo_usuario AS t ON u.id_tipousuario = t.idTipousuario
            WHERE u.idusuarios= '$ID'  ";
$resultado=$conexion->query($sqlu);
			...
			<div id="sidebar" class="sidebar                  responsive                    ace-save-state">
				...
				<?php
					...
					if ($_SESSION['tipo_usuario']==2) {
						require_once("menulateralAlumnos.php");
				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Registro de Clases</li>
						</ul><!-- /.breadcrumb -->
...
<form action="<?php $_SERVER["PHP_SELF"]; ?>" method="POST" >
			...
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Nombre Completo </label>
					<div class="col-sm-9">
					<input type="text" class="form-control"  name="nombre" value="<?php echo $filas['Nombre'] ;?> "  />
					</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Identidad</label>
					<div class="col-sm-9">
					<input type="text" class="form-control" name="identidad"value="<?php echo $filas['Identidad'] ;?> "  />
					</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Correo</label>
					<div class="col-sm-9">
					<input type="email" class="form-control" name="correo" value="<?php echo $filas['Correo'] ;?> "  />
					</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Usuario</label>
					<div class="col-sm-9">
					<input type="text" class="form-control" name="user" value="<?php echo $filas['usuario'] ;?> "  />
					</div>
			</div>
			<div class="form-group">
				<label class="col-sm-3 control-label no-padding-right" for="form-field-1-1"> Contraseña</label>
					<div class="col-sm-9">
				<input type="password" class="form-control" name="pass"value="<?php echo $filas['password'] ;?> "   />
					</div>
			</div>
				...
				<div class="clearfix">
					...
					<button type="submit" name="actualizar"   class="width-65 pull-right btn btn-sm btn-success">
			 ...
			 */
			 ...
			 placeholder.data('draw', drawPieChart);
			  ...
			  placeholder.on('plothover', function (event, pos, item) {
				...
				$.plot("#sales-charts", [
					{ label: "Usuario", data: d1 },
					{ label: "Telefono", data: d2 },
					{ label: "Correo", data: d3 }
				], {
					hoverable: true,
					shadowSize: 0,
					series: {
...
if (isset($_POST["actualizar"])) {
              echo "<script>
                              alert(' No tienes permiso para modificar registros ');
